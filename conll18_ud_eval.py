...
def load_conllu(file):
    ...
    while True:
        ...
        if not line:
            ...
            def process_word(word):
                if word.parent == "remapping":
                    raise UDError("There is a cycle in a sentence, at line ", line_num)
                if word.parent is None:
                    head = int(word.columns[HEAD])
                    if head < 0 or head > len(ud.words) - sentence_start:
                        raise UDError("HEAD '{}' points outside of the sentence, at line {}".format(_encode(word.columns[HEAD], line_num)))
                    if head:
            ...
            if len([word for word in ud.words[sentence_start:] if word.parent is None]) != 1:
                raise UDError("There are multiple roots in a sentence, at line ", line_num)
        ...
        columns = line.split("\t")
        if len(columns) != 10:
            raise UDError("The CoNLL-U line does not contain 10 tab-separated columns: '{}', at line {}".format(_encode(line), line_num))
        ...
        if not columns[FORM]:
            raise UDError("There is an empty FORM in the CoNLL-U file, at line ", line_num)
        ...
        if "-" in columns[ID]:
            try:
                ...
                raise UDError("Cannot parse multi-word token ID '{}', at line {}".format(_encode(columns[ID]), line_num))

            for _ in range(start, end + 1):
                word_line = _decode(file.readline().rstrip("\r\n"))
                word_columns = word_line.split("\t")
                if len(word_columns) != 10:
                    raise UDError("The CoNLL-U line does not contain 10 tab-separated columns: '{}', at line {}".format(_encode(word_line), line_num))
        ...
        # Basic tokens/words
            ...
            try:
                ...
                raise UDError("Cannot parse word ID '{}', at line {}".format(_encode(columns[ID]), line_num))
            if word_id != len(ud.words) - sentence_start + 1:
                raise UDError("Incorrect word ID '{}' for word '{}', expected '{}', at line {}".format(
                    _encode(columns[ID]), _encode(columns[FORM]), len(ud.words) - sentence_start + 1, line_num))

            try:
                head_id = int(columns[HEAD])
            except:
                raise UDError("Cannot parse HEAD '{}', at line {}".format(_encode(columns[HEAD]), line_num))
            if head_id < 0:
                raise UDError("HEAD cannot be negative, at line ", line_num)
...
def evaluate(gold_ud, system_ud):
    ...
    if gold_ud.characters != system_ud.characters:
        ...
        raise UDError(
            ...
            "Starting at {} First 20 differing characters in gold file: '{}' and system file: '{}'".format(
                index,
